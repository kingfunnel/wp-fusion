...
class WPF_Log_Table_List extends WP_List_Table {
	...
	public function column_message( $log ) {
		...
		if ( false !== strpos( $output, ' contact #' ) ) {
			...
			preg_match( '/(?<=#)\w*/', $output, $contact_id );
	...
	protected function user_dropdown() {
		...
		echo wp_kses_post( __( '<p>If you can read this then your site ran out of memory while building the dropdown of log users.</p><p>You can fix this by clicking <strong>Flush All Logs</strong> above, or by increasing your available memory.</p>', 'wp-fusion' ) );
		echo '</p>';

		if ( function_exists( 'ini_get' ) ) {
			$memory_limit = ini_get( 'memory_limit' );
			echo '<p>' . esc_html( 'Current memory limit:', 'wp-fusion' ) . '<code>' . esc_html( $memory_limit ) . '</code></p>';
		}
		echo '</p></div>';
		...
		if ( ! empty( $users ) ) {
			...
			?>
				...
				<select name="user" id="filter-by-user">
					...
					<option<?php selected( $selected_user, '' ); ?> value=""><?php esc_html_e( 'All users', 'wp-fusion' ); ?></option>
