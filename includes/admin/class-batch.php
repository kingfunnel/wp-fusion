...
class WPF_Batch {
	...
	public function batch_init( $hook = false, $args = array() ) {
		...
		wpf_log( 'info', 0, sprintf( __( 'Beginning %1$s batch operation on %2$d %3$s.', 'wp-fusion' ), '<strong>' . $operations[ $hook ]['label'] . '</strong>', count( $objects ), strtolower( $operations[ $hook ]['title'] ) ), array( 'source' => 'batch-process' ) );
