...
class WPF_Lite_Helper {
	...
	public function init() {
		...
		// OAuth URLs.
		add_filter( 'wpf_zoho_auth_url', array( $this, 'lite_auth_url' ) );
		add_filter( 'wpf_hubspot_auth_url', array( $this, 'lite_auth_url' ) );
		add_filter( 'wpf_nationbuilder_auth_url', array( $this, 'lite_auth_url' ) );
		add_filter( 'wpf_salesforce_auth_url', array( $this, 'lite_auth_url' ) );
		add_filter( 'wpf_drift_auth_url', array( $this, 'lite_auth_url' ) );
		add_filter( 'wpf_birdsend_auth_url', array( $this, 'lite_auth_url' ) );
	...
	public function handle_webhooks() {
		...
		if ( isset( $_REQUEST['wpf_action'] ) ) {
			...
			wpf_log( 'error', 0, 'Webhook received but WP Fusion Lite does not support webhooks. Please upgrade to the full version of WP Fusion to sync data bidirectionally with your CRM.', array( 'source' => 'api' ) );
			wp_die( 'Webhook received but WP Fusion Lite does not support webhooks. Please upgrade to the full version of WP Fusion to sync data bidirectionally with your CRM.' );
	 ...
	 */
	public function lite_auth_url( $url ) {
