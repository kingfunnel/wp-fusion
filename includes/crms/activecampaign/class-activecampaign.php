...
class WPF_ActiveCampaign {
	...
	public function load_contact( $contact_id ) {
		...
		// Standard fields.
		foreach ( $response->contact as $field_name => $value ) {

			// These two field names changed with the move to the v3 API.

			if ( 'firstName' === $field_name ) {
				$field_name = 'first_name';
			}

			if ( 'lastName' === $field_name ) {
		...
		if ( ! empty( $response->{'fieldValues'} ) ) {
			...
			// Custom fields.
			foreach ( $response->{'fieldValues'} as $field_object ) {
				...
				foreach ( $contact_fields as $meta_key => $field_data ) {
					...
					if ( true === $field_data['active'] ) {
						...
						if ( $field_object->field === $field_array[0] ) {
