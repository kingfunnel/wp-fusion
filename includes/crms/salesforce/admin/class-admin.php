...
class WPF_Salesforce_Admin {
	...
	public function register_connection_settings( $settings, $options ) {
		...
		if ( empty( $options['sf_access_token'] ) && ! isset( $_GET['code'] ) ) {
			...
			$new_settings['salesforce_header']['desc'] .= '<td><a class="button button-primary" href="' . esc_url( $this->get_oauth_url() ) . '">' . esc_html__( 'Authorize with Salesforce', 'wp-fusion' ) . '</a><br />';
			$new_settings['salesforce_header']['desc'] .= '<span class="description">' . esc_html__( 'You\'ll be taken to Salesforce to authorize WP Fusion and generate access keys for this site.', 'wp-fusion' ) . '</td>';
			$new_settings['salesforce_header']['desc'] .= '</tr></table>';

			if ( ! is_ssl() ) {
				$new_settings['salesforce_header']['desc'] .= '<p class="wpf-notice notice notice-error">' . esc_html__( '<strong>Warning:</strong> Your site is not currently SSL secured (https://). You will not be able to connect to the HubSpot API. Your Site Address must be set to https:// in Settings &raquo; General.', 'wp-fusion' ) . '</p>';
			}
