...
class WPF_Woocommerce extends WPF_Integrations_Base {
	...
	public function order_meta_box_callback( $post ) {
		...
		<?php if ( $contact_id ) : ?>
			...
			<p class="post-attributes-label-wrapper">
				...
				<?php $url = wp_fusion()->crm_base->get_contact_edit_url( $contact_id ); ?>
				<?php if ( false !== $url ) : ?>
		...
		<p class="post-attributes-label-wrapper">
			...
			<a href="<?php echo esc_url( add_query_arg( array( 'order_action' => 'wpf_process' ) ) ); ?>" class="wpf-action-button button-secondary"><?php _e( 'Process WP Fusion actions again ', 'wp-fusion' ); ?></a>
	...
	public function exclude_restricted_products( $posts, $query ) {
		...
		if ( is_admin() || wpf_get_option( 'woo_hide' ) != true ) {
			return $posts;
		}
	...
	public function export_options( $options ) {
		...
		$options['woocommerce'] = array(
			...
			'title'         => 'Orders',
			'process_again' => true,
			'tooltip'       => __( 'Finds WooCommerce orders that have not been processed by WP Fusion, and adds/updates contacts while applying tags based on the products purchased.', 'wp-fusion' ),
		);
	 ...
	 * @return void
	 */
	public function batch_step( $order_id, $args = array() ) {
		...
		$this->process_order( $order_id, $force );
