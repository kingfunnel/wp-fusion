...
class WPF_LearnDash extends WPF_Integrations_Base {
					...
					foreach ( $value['steps']['h'] as $post_type => $posts ) {
						...
						foreach ( $posts as $post_id => $sub_posts ) {
							...
							if ( ! wpf_user_can_access( $post_id ) ) {
								unset( $value['steps']['h'][ $post_type ][ $post_id ] );
								$value['steps_count']--;
								continue;
							}
	...
	public function save_meta_box_data( $post_id ) {
		...
		if ( ! empty( $_POST['learndash-course-access-settings'] ) ) {
			...
			if ( isset( $_POST['learndash-course-access-settings']['step_display'] ) ) {
				$data['step_display'] = $_POST['learndash-course-access-settings']['step_display'];
				unset( $_POST['learndash-course-access-settings']['step_display'] );
			}

			if ( isset( $_POST['learndash-course-access-settings']['lesson_locked_text'] ) ) {
				$data['lesson_locked_text'] = sanitize_text_field( $_POST['learndash-course-access-settings']['lesson_locked_text'] );
				unset( $_POST['learndash-course-access-settings']['lesson_locked_text'] );
			}
		}
