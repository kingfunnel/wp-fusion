...
class WPF_Simply_Schedule_Appointments extends WPF_Integrations_Base {
	...
	public function create_update_customer( $data ) {
		...
		$appointment_data    = $data['appointment'];
		$email               = $appointment_data['customer_information']['Email'];
		...
		if ( empty( $email ) && empty( $user_id ) ) {
			...
			wpf_log( 'error', 0, 'No email address specified for the appointment <a href="' . admin_url( 'admin.php?page=simply-schedule-appointments#/ssa/appointment-types/type/' . $appointment_type_id ) . '" target="_blank">#' . $appointment_type_id . '</a>. Aborting' );
			return false;
	...
	public function add_meta_field_group( $field_groups ) {
		...
		$field_groups['simply-schedule-appointments'] = array(
			'title'  => 'Simply Schedule Appointments',
			'fields' => array(),
		);
