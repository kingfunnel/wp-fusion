	...
	public static function process_form_data( $args ) {
		...
		if ( ! empty( $contact_id ) ) {
			...
			if ( is_wp_error( $contact_id ) ) {
				...
				wpf_log( 'info', $user_id, 'Successfully created contact #' . $contact_id . '.', array( 'source' => sanitize_title( $integration_name ) ) );
		...
		if ( wpf_get_option( 'auto_login_forms', false ) == true && ! wpf_is_user_logged_in() ) {
			...
			wpf_log( 'info', 0, 'Starting auto-login session from form submission for contact #' . $contact_id, array( 'source' => sanitize_title( $integration_name ) ) );
