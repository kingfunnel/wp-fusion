...
class WPF_bbPress extends WPF_Integrations_Base {
	...
	public function begin_redirect() {
		...
		if ( ( bbp_is_forum_archive() && wpf_get_option( 'bbp_lock' ) ) || ( is_object( $post ) && bbp_is_forum( $post->ID ) && wpf_get_option( 'bbp_lock_all' ) ) ) {

			$redirect = wpf_get_option( 'bbp_redirect' );
			...
			if ( wpf_get_option( 'exclude_admins' ) == true && user_can( $user_id, 'manage_options' ) ) {
				return;
			}
			...
			if ( ! empty( $allow_tags ) && empty( array_intersect( $user_tags, $allow_tags ) ) ) {
				wp_redirect( $redirect );
				exit();
			}
		}
