...
class WPF_Woo_Subscriptions extends WPF_Integrations_Base {
	...
	public function init() {
		...
		add_action( 'save_post_shop_subscription', array( $this, 'save_post' ), 10, 3 );
		...
		// CRM fields.
		add_action( 'save_post_product', array( $this, 'save_crm_fields_data' ), 10, 3 );
		add_filter( 'wpf_woocommerce_subscription_sync_fields', array( $this, 'sync_subscription_renewal_dates' ), 10, 2 );
	 ...
	 */
	public function sync_subscription_renewal_dates( $update_data, $subscription ) {
		...
		foreach ( $subscription->get_items() as $line_item ) {
			...
			$update_data[ 'sub_end_' . $product_id ]          = $subscription->get_date( 'end' );
