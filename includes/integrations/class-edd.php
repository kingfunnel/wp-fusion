...
class WPF_EDD extends WPF_Integrations_Base {
	...
	public function customer_edit_link( $customer ) {
		...
		printf( esc_html__( 'Contact ID: #%s', 'wp-fusion' ), esc_html( $contact_id ) );
		...
		if ( false !== $edit_url ) {
			echo '<span class="info-item"><a href="' . esc_url( $edit_url ) . '" target="_blank">' . sprintf( esc_html__( 'View in %s', 'wp-fusion' ), esc_html( wp_fusion()->crm->name ) ) . '&rarr;</a></span>';
		}
	 ...
	 * @return void
	 */
	public function batch_step( $payment_id, $args = array() ) {
		...
		$this->complete_purchase( $payment_id, true, $force );
