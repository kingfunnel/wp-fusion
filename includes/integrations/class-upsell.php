...
class WPF_Upsell extends WPF_Integrations_Base {
	...
	public function get_contact_id( $order ) {
		...
		if ( property_exists( $order, 'customer' ) && null !== $order->customer ) {
			...
			if ( empty( $contact_id ) ) {
				$contact_id = $order->customer->getAttribute( '_' . wp_fusion()->crm->slug . '_contact_id' );
			}
		}
		...
		if ( empty( $contact_id ) ) {
			...
			if ( false !== $contact_id ) {
				if ( property_exists( $order, 'customer' ) && null !== $order->customer ) {
					$order->customer->setAttribute( '_' . wp_fusion()->crm->slug . '_contact_id', $contact_id );
				}
			}
		}
	...
	public function create_update_customer( $order ) {
		...
		if ( empty( $contact_id ) ) {
			...
			$note_args = array(
				...
				'comment_content' => wp_fusion()->crm->name . ' contact ID ' . $contact_id . ' created via guest-checkout.',
				'user_id'         => $user_id,
			);
		...
		update_post_meta( $order_id, '_' . wp_fusion()->crm->slug . '_contact_id', $contact_id );
