...
class WPF_Users_Insights extends WPF_Integrations_Base {
	...
	public function module_init() {
		...
		$meta_query = new USIN_Meta_Query( wp_fusion()->crm->slug . '_tags', 'serialized_multioption' );
		$meta_query->init();
	...
	public function users_raw_data( $data ) {
		...
		foreach ( $data as $i => $user ) {
			...
			$tags = maybe_unserialize( $user->{ wp_fusion()->crm->slug . '_tags' } );

			if ( ! empty( $tags ) ) {
				$tags = array_map( array( wp_fusion()->user, 'get_tag_label' ), $tags );
				$data[ $i ]->{ wp_fusion()->crm->slug . '_tags' } = implode( ', ', $tags );
